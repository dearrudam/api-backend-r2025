...
public class RemotePaymentProcessor {
    ...
    @Inject
    public RemotePaymentProcessor(PaymentRepository paymentRepository,
    ...
    public void process(PaymentRequest paymentRequest) {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                paymentRepository.saveAsDefaultPayment(payment);
    ...
    private void processFallback(Payment payment) throws IOException, InterruptedException {
        ...
        var response = httpClient.send(request, HttpResponse.BodyHandlers.discarding());
        if (response.statusCode() == 200) {
            paymentRepository.saveAsFallbackPayment(payment);
        } else {
